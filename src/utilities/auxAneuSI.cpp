...
#include <vtkWindowToImageFilter.h>  // TESTING RENDERING
...
void CenterLinesLocator(bool verbose, bool *line2, bool *line3, vtkSmartPointer<vtkPolyData> clines, vtkSmartPointer<vtkPlane> neckPlane, int clineTags[])
{
    ...
    for ( int i=0; i<clines->GetNumberOfLines(); i++ )
    {
        ...
        if ( pointDist < 3.0*pointRadius )  // Discard "type 3" centerlines from the analysis
        {
...
void FindBranchClipPoints( bool verbose, vtkSmartPointer<vtkPolyData> clines,  int clineTags[], vtkSmartPointer<vtkPlane> neckPlane,
                   ...
                   vtkSmartPointer<vtkIdList> clipDirections, vtkSmartPointer<vtkCleanPolyData> clean)
{
    ...
    int    dir;
    int    numPts;
    double p1a[3], p1b[3], p1c[3];  // Cylinder base and normal points <--p1b--p1a--p1c-- for mean tangents to cline
    double p1tangF[3], p1tangB[3];  // "Forward" and "Backward" tangents
    double p1tang[3], p2tang[3];    // Tangents
    double reltol;                  // Tolerance for cleaning
    int    iend;    
    double area_ratio;

    // Coordinates of slices points (reference and radially displaced)
    ...
    for (int i=0; i<clipPoints->GetNumberOfIds(); i++)
    {
        ...
        while (iend == 0)
        {
            ...
            {        
                ...
                clines->GetPoint(p1Id+1,p1c);   // p1c coordinates
            }
            else 
            {
                clines->GetPoint(p1Id+1,p1b);   // p1b coordinates
                clines->GetPoint(p1Id-1,p1c);   // p1b coordinates
            }

            // Get two adyacent tangents, keep the mean between them
            vtkMath::Subtract(p1b,p1a,p1tangF);  // Get p1tang
            vtkMath::Subtract(p1a,p1c,p1tangB);  // Get p1tang
            vtkMath::Add(p1tangF,p1tangB,p1tang);

            p1tang[0] = p1tang[0] * 0.5;
            p1tang[1] = p1tang[1] * 0.5;
            p1tang[2] = p1tang[2] * 0.5;
            ...
            clean->SetAbsoluteTolerance(0.005);
        ...
        // Check if points are connected in the slice
        int  pt_count[numPts];
        bool fix_slice = false;

        Fill(numPts,0,pt_count);
        ...
        vtkSmartPointer<vtkCellArray> sliceCells = vtkSmartPointer<vtkCellArray>::New();
        sliceCells = clean->GetOutput()->GetLines();

        for (int i=0; i<sliceCells->GetNumberOfCells(); i++)
        {
            clean->GetOutput()->GetCellPoints(i,pointIds);
            pt_count[pointIds->GetId(0)] += 1;
            pt_count[pointIds->GetId(1)] += 1;
        }
        ...
        for( int id=0; id < numPts; id++ )
        {
            if (pt_count[id] == 1)
            {
                newCellIds->InsertNextId(id);
                fix_slice = true;
            }
        }

        //Fix slice, adding the missing nodes connectivity
        vtkSmartPointer<vtkLine> newLine = vtkSmartPointer<vtkLine>::New();

        vtkSmartPointer<vtkPolyData> slicePdt = vtkSmartPointer<vtkPolyData>::New();


        if (fix_slice==true)
        {
            slicePdt->SetPoints(clean->GetOutput()->GetPoints());

            newLine->GetPointIds()->SetId(0, newCellIds->GetId(0));
            newLine->GetPointIds()->SetId(1, newCellIds->GetId(1));

            sliceCells->InsertNextCell(newLine);

            slicePdt->SetLines(sliceCells);
        ...
        {
            slicePdt->ShallowCopy(clean->GetOutput());
            slicePdt->BuildLinks();
            slicePdt->BuildCells();
        }
        ...
        //Sort edges and points in the slice
        int ptId=0, cellId=0;
        ...
        {           
            ...
            slicePdt->GetPointCells(topPts->GetId(id),cellIds);
            ...
            slicePdt->GetCellPoints(topCells->GetId(id+1),ptIds);
        ...
        sliceCenter->SetInputData(slicePdt);
        ...
        {
            slicePdt->GetPoints()->GetPoint(i,refPt);
        ...
        recurClean->GetOutput()->BuildCells();
        recurClean->GetOutput()->BuildLinks();

        // For testing purposes only, uncomment if needed
        // vtkSmartPointer<vtkPolyDataWriter> clipWriter = vtkSmartPointer<vtkPolyDataWriter>::New();
        // clipWriter->SetInputData(recurClean->GetOutput());
        // clipWriter->SetFileName("clipPartial.vtk");
        // clipWriter->Write();
        // clipWriter->Update();
